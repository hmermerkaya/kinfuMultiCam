...
#ifndef KINFU_INCLUDE_KINFU_GPU_KINFU_H_
#define KINFU_INCLUDE_KINFU_GPU_KINFU_H_
...
//#include <gpu/color_volume.h>
//#include <gpu/raycaster.h>

#include <gpu/cyclical_buffer.h>
    ...
    namespace gpu
    {
      ...
      class PCL_EXPORTS KinfuTracker
      {
        public:
          ...
          //typedef kinfu::gpu::PixelRGB PixelRGB;
...
//          performLastScan (){perform_last_scan_ = true; PCL_WARN ("Kinfu will exit after next shift\n");}
         ...
         //kinect_v1
          ...
          // kinect_v2
          KinfuTracker (const Eigen::Vector3f &volumeSize, const float shiftingDistance, int rows = 424, int cols = 512);
...
//          }
          ...
          void
		  setRMatAndTVec(std::vector<Eigen::Matrix<float, 4, 4, Eigen::RowMajor>, Eigen::aligned_allocator<Eigen::Matrix<float, 4, 4, Eigen::RowMajor>>> tmats);
        ...
        private:
          ...
          convertTransforms (Matrix3frm& transform_in_1, Matrix3frm& transform_in_2, Eigen::Vector3f& translation_in_1, Eigen::Vector3f& translation_in_2,
                                         kinfu::device::Mat33& transform_out_1, kinfu::device::Mat33& transform_out_2, float3& translation_out_1, float3& translation_out_2);
          ...
          convertTransforms (Matrix3frm& transform_in_1, Matrix3frm& transform_in_2, Eigen::Vector3f& translation_in,
                                         kinfu::device::Mat33& transform_out_1, kinfu::device::Mat33& transform_out_2, float3& translation_out);
          ...
          convertTransforms (Matrix3frm& transform_in, Eigen::Vector3f& translation_in,
                                         kinfu::device::Mat33& transform_out, float3& translation_out);
          ...
          /** \brief helper function that pre-process a raw detph map the kinect fusion algorithm.
            ...
            */
          inline void
          prepareMaps (const DepthMap& depth_raw, const kinfu::device::Intr& cam_intrinsics);
          ...
          /** \brief helper function that performs GPU-based ICP, using vertex and normal maps stored in v/nmaps_curr_ and v/nmaps_g_prev_
            ...
            */
//          inline bool
//          performICP(const kinfu::device::Intr& cam_intrinsics, Matrix3frm& previous_global_rotation, Vector3f& previous_global_translation, Matrix3frm& current_global_rotation, Vector3f& current_global_translation);
          ...
          /** \brief helper function that performs GPU-based ICP, using the current and the previous depth-maps (i.e. not using the synthetic depth map generated from the tsdf-volume)
            ...
            */
//          inline bool
//          performPairWiseICP(const kinfu::device::Intr cam_intrinsics, Matrix3frm& resulting_rotation, Vector3f& resulting_translation);
...
#endif /* KINFU_INCLUDE_KINFU_GPU_KINFU_H_ */
