...
#ifndef KINFU_SRC_INTERNAL_H_
#define KINFU_SRC_INTERNAL_H_


#include <gpu/safe_call.hpp>
#include <device.h>
#include <gpu/tsdf_buffer.h>
...
//using namespace pcl::gpu;
  ...
  namespace kinfu
  {
    namespace device
    {
      ...
      void
	  transformCloudGPU(const  DeviceArray<float3> &cloudIn, DeviceArray<float3> &cloudOut,const Mat33& Rmat, const float3& tvec );
     ...
     KF_EXPORTS
      ...
      integrateTsdfVolume (const PtrStepSz<ushort>& depth, const Intr& intr, const float3& volume_size,
                          const Mat33& Rcurr_inv, const float3& tcurr, float tranc_dist, PtrStep<short2> volume, const kinfu::gpu::tsdf_buffer* buffer, DeviceArray2D<float>& depthScaled);
      ...
      /** \brief Function that clears the TSDF values. The clearing takes place from the origin (in indices) to an offset in X,Y,Z values accordingly
        ...
        */
      KF_EXPORTS void
      clearTSDFSlice (PtrStep<short2> volume, kinfu::gpu::tsdf_buffer* buffer, int shiftX, int shiftY, int shiftZ);
      ...
      raycast (const Intr& intr, const Mat33& Rcurr, const float3& tcurr, float tranc_dist, const float3& volume_size,
              const PtrStep<short2>& volume, const kinfu::gpu::tsdf_buffer* buffer, MapArr& vmap, MapArr& nmap);
      ...
      /** \brief Perform point cloud extraction of a slice from tsdf volume
        ...
        */
      KF_EXPORTS size_t
      extractSliceAsCloud (const PtrStep<short2>& volume, const float3& volume_size, const kinfu::gpu::tsdf_buffer* buffer, const int shiftX, const int shiftY, const int shiftZ, PtrSz<PointType> output_xyz, PtrSz<float> output_intensities);
